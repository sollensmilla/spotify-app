import {
  Chart as ChartJS,
  LinearScale,
  PointElement,
  Tooltip,
  Legend,
} from "chart.js";

import ChartScatter from "./ChartScatter";
import { calculateAverages } from "./chartUtils";
import LegendItem from "./LegendItem";

ChartJS.register(LinearScale, PointElement, Tooltip, Legend);

/**
 * Renders a scatter chart visualizing the relationship between tempo and energy of tracks.
 * @param {{ tracks: Array }} param0 - The props object containing the list of tracks.
 * @returns {JSX.Element|null} - The rendered ChartView component or null if no tracks are provided.
 */
export default function ChartView({ tracks }) {
  if (!tracks.length) return null;

  const { avgTempo, avgEnergy } = calculateAverages(tracks);

  const colors = ["#ef4444", "#3b82f6", "#22c55e", "#a855f7"];

  const data = {
    datasets: [
      {
        label: "Tracks",
        data: tracks.map((t) => ({
          x: t.tempo,
          y: t.energy,
        })),
        pointBackgroundColor: tracks.map(
          (t) => colors[t.cluster] || "#999"
        ),
        pointRadius: tracks.map((t) => t.popularity / 20),
      },
      {
        label: "Average",
        data: [{ x: avgTempo, y: avgEnergy }],
        pointRadius: 8,
        pointBackgroundColor: "black",
      },
    ],
  };

  const options = {
    scales: {
      x: { title: { display: true, text: "Tempo (BPM)" } },
      y: { title: { display: true, text: "Energy" }, min: 0, max: 1 },
    },
    plugins: {
      tooltip: {
        callbacks: {
          label: function (context) {
            const track = tracks[context.dataIndex];

            if (!track) {
              return `Average — ${avgTempo.toFixed(0)} BPM, Energy: ${avgEnergy.toFixed(2)}`;
            }

            return `${track.track_name} — ${track.tempo} BPM, Energy: ${track.energy} (Cluster ${track.cluster})`;
          },
        },
      },
    },
  };

  return (
    <div style={{ marginTop: "2rem" }}>
      <h3>Visualization</h3>

      <div
        style={{
          display: "flex",
          gap: "1rem",
          justifyContent: "center",
          marginBottom: "0.5rem",
        }}
      >
        <LegendItem color="#ef4444" label="Cluster 0" />
        <LegendItem color="#3b82f6" label="Cluster 1" />
        <LegendItem color="#22c55e" label="Cluster 2" />
        <LegendItem color="black" label="Average" />
      </div>

      <ChartScatter data={data} options={options} />
    </div>
  );
}